...
/// <summary>
/// Start-with-Windows. A normal install uses HKCU\...\Run; an elevated install uses a
/// highest-run-level scheduled task, because Windows silently skips elevated apps in the
/// Run key. Nothing is mirrored into config.json, so the checkbox stays honest if the
/// entry is removed by something else.
/// </summary>
internal static class StartupRegistry
{
    ...
    /// the elevation state has changed. Also drops the pre-rebrand BlancoShot value,
    /// which points at an exe that no longer exists.
    /// </summary>
