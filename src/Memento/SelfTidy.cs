...
/// <summary>
/// Cleans up after browser re-downloads: a fresh copy lands as "Memento (1).exe" next to
...
internal static class SelfTidy
{
    ...
    /// <summary>"Memento (2)" → "Memento"; names without a copy suffix pass through.</summary>
    ...
    public static void Run()
    {
        try
        {
            ...
            foreach (var file in Directory.EnumerateFiles(dir, baseName + "*.exe"))
            {
                ...
                // Exactly "base.exe" or "base (N).exe" — "Memento-self-contained.exe"
                // is a different artifact, not a duplicate of "Memento.exe".
